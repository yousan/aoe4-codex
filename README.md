...
`civs.html`。文明を選ぶと、その文明の**特性**（ゲーム内の文明概要パネルの項目）と、
**その文明だけが研究できるテクノロジー**が時代ごとに並ぶ。

- テクノロジーは 1件＝1カード。**研究施設・コスト・研究時間・効果**を出す。
  効果はゲーム内のツールチップそのまま
- 上の帯のチェックボックスで**研究施設で絞り込める**。状態は URL に入る（`?civ=mac&b=varangian-arsenal`）
- 銀（マケドニア王朝）・宰相ポイント（オスマン）のような固有資源のコストもそのまま出す
- マケドニア王朝のヴァリャーグ造兵廠の段階研究は「全6段階・この時代で2回」として1枚にまとめている
- ゲーム内に対応する文言が見つからなかったものは英語のまま出し、`EN` の印を付ける
  （現状 166件中2件。テクノロジー430件はすべてゲーム内表記）
...
- **どの文明の固有技術か・時代・コスト・研究施設**は upstream の
  [aoe4world/data](https://github.com/aoe4world/data)（`technologies/all.json` と
  `civilizations/<civ>.json`）。リポジトリに同梱している
- **文言はゲーム本体から取っている**。ユニット名と同じ方針で、こちらでは訳さない
  1. `Attrib.sga` の `attrib\upgrade\**\*.rgd`（能力は `ebps` 側）を開いて
     `screen_name` と `help_text_formatter.formatter` の文字列IDを読む
  2. その ID を各言語の `Locale*.sga` で引く
  3. ゲーム側の説明文は `%1%` のようなプレースホルダ入りなので、
     **英語テンプレートを upstream の解決済み英文に突き合わせて数値を取り出し**、各言語版に埋める
- 「固有」の判定は「upstream の `unique` フラグが立っている」**または**
  「その技術を持つ文明が1つだけ」。フラグだけだとアッバース朝／アイユーブの
  知恵の館のウィング技術がこぼれ、逆にファランクス（ビザンティン＋マケドニア王朝）のように
  派生文明と共有していてフラグだけ立っているものもある
- 時代到達で自動的に付くだけで研究できないもの（`Castle Age King` など）は出さない
- 生成は `python3 tools/build_civs.py`（要: ゲーム本体）
...
- 固有テクノロジーとユニット一覧を相互にリンクする（「この技術はどのユニットに効くか」）
- 歴史的建造物（ランドマーク）の一覧
