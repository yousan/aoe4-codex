...
- 鹿と果実の茂みは「大(7頭) × 4群 = 28頭」のように、サイズ・群れ数・総数を分けて出す。
  鹿の群れは 小3/中5/大7/特大10頭、果実のパッチは 小2/中6/大8個
- **初期の鹿が自陣の近くに湧くか**を 近い / やや近い / 遠い で出す。
  wiki に記述が無いマップは「記述なし」とし、憶測では埋めない
...
- 更新は `python3 tools/build_maps.py`。プール・資源データ・画像・日本語名を全部取り直す。
  **月初のローテーション後に走らせる**
- 手書きの解説文と「初期の鹿」の判定だけは `tools/maps-notes.ja.json` に分けてある。
  数値はここに書かない（数値は必ず wiki 由来にして、解説と事実が混ざらないようにしている）
