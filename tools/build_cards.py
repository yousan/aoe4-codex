...
SAMPLES = [('Knight', 3), ('Spearman', 1), ('Crossbowman', 3), ('Archer', 2),
           ('Man-at-Arms', 3), ('Horseman', 2), ('Camel Archer', 2), ('War Elephant', 3),
           ('Mangonel', 3), ('Streltsy', 4), ('Galleass', 3), ('Villager', 1)]
cards = [BY[k] for k in SAMPLES if k in BY]
pairs = [BY[('Knight', 3)], BY[('Spearman', 1)]]

legend_stats = ''.join(f'<div class="lg">{ico(k)}<span>{v}</span></div>'
                       for k, v in STAT_JP.items() if k != 'i-fire')
legend_attrs = ''.join(
    f'<div class="lg">{"<span class=kj>"+("重" if k=="a-heavy" else "軽")+"</span>" if k in ("a-heavy","a-light") else ico(k)}<span>{v}</span></div>'
    for k, v in ATTR_JP.items())

miss = sorted({u['n'] for u in UNITS if not u['jp']})

HTML = f'''<!DOCTYPE html><html lang="ja"><head><meta charset="utf-8">
<title>AoE4 ユニットカード v3</title><style>
...
<div class="sec"><h2>日本語名＋属性フル表示 <small>属性はゲーム内のクラスそのまま（ボーナスの対象になる単位）</small></h2>
<div class="row">{''.join(card(u) for u in cards)}</div></div>

<div class="sec"><h2>重装／軽装の見せ方 3案 <small>2.2倍表示。左=騎士（重装）右=槍兵（軽装）</small></h2>
<div class="zwrap">
  <div class="zcol"><h3>案1: 線の太さ</h3><div class="zoom">{card(pairs[0], 'stroke')}</div></div>
  <div class="zcol"><h3>案2: 漢字チップ</h3><div class="zoom">{card(pairs[0], 'kanji')}</div></div>
  <div class="zcol"><h3>案3: 塗りバッジ</h3><div class="zoom">{card(pairs[0], 'badge')}</div></div>
</div>
<div class="zwrap">
  <div class="zcol" style="height:400px"><div class="zoom">{card(pairs[1], 'stroke')}</div></div>
  <div class="zcol" style="height:400px"><div class="zoom">{card(pairs[1], 'kanji')}</div></div>
  <div class="zcol" style="height:400px"><div class="zoom">{card(pairs[1], 'badge')}</div></div>
</div></div>

<div class="sec"><h2>属性アイコン凡例 <small>ダメージボーナスの対象になるやつ</small></h2>
<div class="lgwrap">{legend_attrs}</div></div>

<div class="sec"><h2>ステータスアイコン凡例</h2>
<div class="lgwrap">{legend_stats}</div>
<div class="note">日本語名は AoE4攻略Wiki（AoE Haul）とゲーム内表記から。未対応の名前は英語のまま出る（現在 {len(miss)} 件）。</div>
</div>
</body></html>'''

open(os.path.join(ROOT, 'aoe4-card.html'), 'w', encoding='utf-8').write(HTML)
print('units', len(UNITS), 'jp', sum(1 for u in UNITS if u['jp']), 'missing-jp', len(miss))
print('samples', [(u['n'], u['jp'], u['at']) for u in cards[:6]])
